...
def locate(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN: # if user clicks
        ...
        d2 = ((particle_end[0] - particle_start[0])**2) 
              + ((particle_end[1] - particle_start[1])**2)
        particle_radius = (d2**(0.5))/2
        cv2.circle(frame, particle_center, int(particle_radius+0.5), (255,0,0), 1)        # draw circle that shows the radius and location of cirlces that the Hough circle transform will search for
